...
- **Alte MUSE-Next.js-App stilllegen** (Schritt 10): nicht relevant, solange die AIMUSED-Panels dem MUSE-Funktionsumfang noch nicht gleichwertig sind (u. a. wegen des obigen Punkts).
...
**Build/Test-Status dieser Runde**: `npx turbo build`, `npx turbo test` und `npx turbo lint` laufen grün (`exit 0`) für alle sechs Workspace-Pakete (`@signal-app/core`, `@signal-app/player`, `dialog-hooks`, `@signal-app/orchestration-core`, `@signal-app/midi-project`, `signal`/`app`) — mit denselben vorbestehenden, nicht mit dieser Änderung zusammenhängenden Lint-Abweichungen wie in Runde 1 (`packages/dialog-hooks`, zusätzlich `packages/player`: beides Prettier-Formatierungsabweichungen, durch `git stash` verifiziert als bereits vor dieser Runde vorhanden, unberührt von dieser Aufgabe). App-Lint-Baseline vs. nach dieser Runde: 360 → 366 geprüfte Dateien, 86 → 87 Biome-*Warnungen* (ein einziger neuer Eintrag, derselben bereits vorbestehenden `useExhaustiveDependencies`-Art wie die drei existierenden Fälle in `OnInit.tsx`), 13 → 13 Infos, in beiden Fällen `exit 0`. `npm run test -w app` stieg von 35 auf 41 grüne Tests.
