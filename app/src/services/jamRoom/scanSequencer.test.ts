...
describe("scan sequencer", () => {
  it("creates evenly distributed scan lines", () => {
    ...
    expect(lines.map((line) => line.yRatio)).toEqual([
  ...
  it("does not trigger on the initialization frame", () => {
    const result = advanceScanLines(createScanLines(1), [220], 16, 400, {
      ...DEFAULT_SCAN_CONFIG,
      lineCount: 1,
    })
  ...
  it("emits one event when a brightness pulse crosses the trigger line", () => {
    ...
    const crossed = advanceScanLines(activated.lines, [100], 50, 100, config)
